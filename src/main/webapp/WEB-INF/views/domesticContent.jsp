...
function re_comment(CMTNO) {
	...
	/* alert(data); */
		...
		var html  = "<form id='commentForm' name='commentForm' method='post'>";
			...
			html += "<a onclick='fn2_comment("+${boardDto.textNo}+")' class='button'>등록</a>";
	 		html += "</div>";
	 		html += "</div>";
	 		html += "</form>";
		...
		success : function(data) {
			...
			if( data.length > 0 ) {
				...
				for( i=0; i < data.length; i++ ) {
					...
					html += "<span class='leftc2'>"+data[i].MEMID+" ( "+data[i].MEMNAME+" )"+"</span>";
					/* html += "<span class='middlec2'>ㅣ</span>"; */
					html += "<span class='rightc2'>"+data[i].CMTDATE+"</span>";
					...
					html += "<span class='rightc3'><a href='#' onclick='re_comment("+data[i].CMTNO+")' >답글</a></span>"; 
					html += "</div>";
					html += "<div class='cmtbody'>";
					html += "<span class='leftc3'>"+data[i].CMTTEXT+"</span>";
					html += "</div>";
					html += "<div id='reCmt'>";
					html += "</div>";
				}
			...
			<div class="readForm">
				...
				<c:if test="${detailDto != null }">
					<div class="row">
						...
						<div class="middle">${detailDto.startDate} ~ ${detailDto.endDate}</div>
					</div>
					...
					<div class="row">
						<div class="left">베스트</div>
						<div class="middle">${detailDto.myBest}</div>
					</div>
					
					<div class="row">
						<div class="left">워스트</div>
						<div class="middle">${detailDto.myWorst}</div>
					</div>
				</c:if>
				...
				<div class="comment">
					...
					<form id="commentForm" name="commentForm" method="post">
						<div class="cmtbody2">
							...
							<div class="rightc6">
								<a onclick="fn_comment('${boardDto.textNo}')" id="reIst" class="button">등록</a>
							</div>
						</div>
						<input type="hidden" id="b_code" name="bod_no" value="${boardDto.textNo}">
					</form>
				...
				<div class="cmmntyLower1">
					<c:if test="${MEM_NUM == boardDto.mbrCode}">
						<input type="button" id="updatePost" value="수정" onclick="document.location.href='updateDomestic?value=${boardDto.textNo}'">
						<input type="button" id="deletePost" value="삭제" onclick="document.location.href='deletePost.do?value=${boardDto.textNo}'">
					</c:if>
				</div>
				
				<div class="cmmntyLower2">
					<input type="button" id="selectAll" value="목록" onclick="document.location.href='domesticBoard.do'">
				</div>
