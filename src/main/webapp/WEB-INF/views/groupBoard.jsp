...
<link rel="stylesheet" href="resource/css/fonts.css" type="text/css"
	media="all" />
<link rel="stylesheet" href="resource/css/abcss.css" type="text/css"
	media="all" />
<link rel="stylesheet"
...
<body>
	...
	<div class="wrapper">
		<div id="grouppage" class="container">
			<div class="title">
				<h2><span style="color: skyblue;">Go Together ෆ</span></h2>	
						...
						<c:forEach  var="dto" items="${list}" varStatus="status">	
							<c:if test="${'9' ne CmonDto.bod_delchk}">
								<tr class="selCont" id="selCont">
									<td class="tabletd">${dto.trvArea}</td>
									<td class="titletd">
										<c:if test="${MEM_NUM != null and '9' ne MEM_LVL}">
											<a href="groupContent.do?value=${dto.textNo}&mCode=${dto.mbrCode}">${dto.title}</a>
										</c:if>
										<c:if test="${MEM_NUM == null or '9' eq MEM_LVL}">
		...
		<form action="groupContent.do" method="post" id='viewContent'>
		...
		<div id="divLower">
			<div id="divPage">
				...
				<%-- <div class="writego">
					<c:if test="${MEM_NUM != null and '9' ne MEM_LVL}">
						<button class="insertGo" type="button"
							onclick="document.location.href='/WebProjectV201/groupWrite'">모집하기</button>
					</c:if>
				</div> --%>
				
				<div id="btnWrite">
						<c:if test="${MEM_NUM != null and '9' ne MEM_LVL}">
							<a href="groupWrite" id="insertPost" class="button">글쓰기</a>
						</c:if>
						<c:if test="${MEM_NUM == null or '9' eq MEM_LVL}">
				...
				<form action="groupBoard.do" method="post" id='frmPaging'>
